...
from chapter4.redis_queue import RedisQueue
...
def threaded_crawler_rq(start_url, link_regex, user_agent='wswp', proxies=None,
    ...
    """
    Crawl from the given start URLS following links matched by link_regex. In this implementation,
    we do not actually scrape any information.
    :param start_url: (str or list of strs) web site(s) to start crawl
    :param link_regex: (str) regex to match for links
    :param user_agent: (str) user agent (defaut: wswp)
    :param proxies: (list of dicts): a list of possible dicts for http/https proxies
    ...
    :param num_retries: (int): # of retries when 5xx error (default: 2)
    :param cache: (dict) cache dict with urls as keys and dicts for responses (default: {})
    :param max_threads:
    :param scraper_callback: function to be called on url and html content
    :return:
    """
    crawl_queue = RedisQueue()
    crawl_queue.push(start_url)
    # keep track of which URL's have been seen before
    robots = {}
...
if __name__ == '_main__':
    from chapter4.alexa_callback import AlexaCallback
    from chapter3.rediscache import RedisCache
    import argparse
    ...
    parser.add_argument('max_threads', type=int, help='maximum number of threads',
                        nargs='?', default=5)
    parser.add_argument('num_procs', type=int, help='number of prcoesses',
                        nargs='?', default=None)
    ...
    mp_threaded_crawler(AC.urls, par_args.url_pattern, cache=RedisCache(),
                        num_procs=par_args.num_proces, max_threads=par_args.max_threads)
